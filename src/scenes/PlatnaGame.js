...
export default class Game extends Phaser.Scene {
    ...
    create() {
        ...
        this.bedna.body.setMass(1.5); // Nebo zkus i vyšší hodnotu, např. 3 nebo 4
    ...
    update(time, delta) {
        ...
        if (this.cursors.left.isDown || this.moveLeft) {
            ...
            this.bedna.body.applyForce = new Phaser.Math.Vector2(-this.pushForce, 0);
    ...
    applyPushImpulse(duration, directionX = 0) {
        if (this.bedna && this.bedna.body) {
            const fixedDuration = 1050; // Dočasná konstanta pro dobu trvání (ms)
            const pushForce = fixedDuration / 500;
            const impulseVelocity = this.pushDirectionX * pushForce;
            console.log('Dočasná duration:', fixedDuration, 'pushForce:', pushForce, 'impulseVelocity:', impulseVelocity / 100);
            //this.bedna.body.setVelocityX(impulseVelocity);
            this.bedna.body.setVelocityX(impulseVelocity / 100);
